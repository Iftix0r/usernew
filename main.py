...
class AccountConfig:
    ...
    def _load_keywords(self):
        """Yo'lovchi va haydovchi kalit so'zlarni umumiy bazadan yuklash"""
        try:
...
def create_message_handler(acc: AccountConfig):
    ...
    async def message_handler(event):
        ...
        if FAST_GROUP_ID and len(text_content) <= 80 and not event.message.sticker:
            ...
            if not _has_emoji:
                try:
                    ...
                    # Bot yoki kanal bo'lsa tez guruhga yuborma
                    if _sender and not getattr(_sender, 'bot', False) and hasattr(_sender, 'first_name'):
                        ...
                        _btn_url = f"https://t.me/{_uusername}" if _uusername else None
                        _buttons = []
                        if _btn_url:
                            _buttons.append([{"text": f"👤 {_uname}", "url": _btn_url}])
                        _buttons.append([{"text": "✅ Zakaz ketadi", "url": f"https://t.me/{acc.bot_username}?start=fastsend_{_uid}_{event.chat_id}_{event.id}"}])
        ...
        # Matndan @username qidirish (telefon yo'q bo'lsa ishlatiladi)
        text_usernames = re.findall(r'@(\w+)', text_content)
        ...
        # 1-bosqich: haydovchi so'zi bo'lsa - ignore
        has_driver_words = any(w.lower() in text_lower for w in acc.keywords['driver'])
        if has_driver_words:
            matched_d = [w for w in acc.keywords['driver'] if w.lower() in text_lower]
            print(f"🚗 AKK#{acc.profile_id}: HAYDOVCHI - IGNORE | {clean_user_name or 'Noma\'lum'} | kalit: {matched_d[0] if matched_d else '?'} | {text_content[:50]}")
            return

        # 2-bosqich: yo'lovchi so'zi bo'lmasa - ignore
        has_passenger_words = any(w.lower() in text_lower for w in acc.keywords['passenger'])
        if not has_passenger_words:
            print(f"❌ AKK#{acc.profile_id}: YO'LOVCHI SO'ZI YO'Q - IGNORE | {clean_user_name or 'Noma\'lum'} | {text_content[:50]}")
            return

        matched = [w for w in acc.keywords['passenger'] if w.lower() in text_lower]
        print(f"✅ YO'LOVCHI ANIQLANDI | {clean_user_name or 'Noma\'lum'} | kalit: {matched[0] if matched else '?'} | MATN: [{text_lower}]")
        ...
        try:
            ...
            # acc.order_group_id faqat baza bo'sh bo'lganda fallback sifatida qo'shiladi
            if not all_order_groups and acc.order_group_id:
                ...
                for gid in all_order_groups:
                    try:
                        ...
                        if phones:
                            ...
                            for tu in text_usernames:
                                message_parts.append(f"🤙 @{tu}")
                        ...
                        elif not phone_to_call and text_usernames:
                            for tu in text_usernames:
                                inline_buttons.append([{"text": f"🤙 @{tu}", "url": f"https://t.me/{tu}"}])
                        ...
                        async with resp:
                            ...
                            if resp.status == 200:
                                ...
                                # mijoz_text userbot orqali yuborilmaydi - FloodWait sabab
                            else:
                                ...
                                if resp.status == 400 and "chat not found" in response_text.lower():
                                    ...
                                    try:
                                        with get_main_db() as _conn:
                                            _conn.execute('DELETE FROM order_groups WHERE group_id = ?', (gid,))
                                            _conn.commit()
                                        logger.warning(f"Guruh bazadan o'chirildi (topilmadi): {gid}")
                                    except Exception as _del_err:
                                        logger.error(f"Guruh o'chirishda xatolik: {_del_err}")
                    except Exception as e:
...
def create_chat_action_handler(acc: AccountConfig):
    ...
    async def _groups(event):
        if event.is_private and event.text and event.text.strip() == '/groups':
            try:
                ...
                logger.error(f"_groups: {e}")
    
    async def _help(event):
        if event.is_private and event.text and event.text.strip() == '/help':
            try:
...
def register_account_commands(c, acc):
    ...
    c.add_event_handler(_groups, events.NewMessage(pattern=r'/groups'))
    c.add_event_handler(_help, events.NewMessage(pattern=r'/help'))
...
async def main():
    ...
    # Default yo'lovchi kalit so'zlar
    try:
        with get_main_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM keywords WHERE type = ?', ('passenger',))
            if cursor.fetchone()[0] == 0:
                for w in ["kerak", "borish kerak", "ketish kerak", "taksi kerak", "mashina kerak", "olib keting", "olib ket"]:
    ...
    try:
        if profiles:
            ...
            if not await client.is_user_authorized():
                try:
                    phone_input = input("Telefon raqamingiz (+998xxxxxxxxx): ").strip()
                except EOFError:
                    raise RuntimeError("Telefon raqamini kiritishda STDIN yo'q yoki yopilgan")
                if not phone_input:
                    raise ValueError("Telefon raqami bo'sh. Iltimos, +998... formatida kiriting.")
                ...
                try:
                    ...
                    raise RuntimeError("Telegram kodini kiritishda STDIN yo'q yoki yopilgan")
                if not code:
                    raise ValueError("Telegram kodi bo'sh. Iltimos, kodni kiriting.")
                ...
                try:
                    ...
                    try:
                        ...
                        raise RuntimeError("2FA parolini kiritishda STDIN yo'q yoki yopilgan")
                    if not password:
                        raise ValueError("2FA paroli bo'sh. Iltimos, parolni kiriting.")
