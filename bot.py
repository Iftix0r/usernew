...
def init_keywords_db():
    ...
    cursor.execute('''
        ...
        VALUES (?, ?)
    ''', ('order_message_header', '🆕 <b>Yangi buyurtma</b>'))
...
def words_menu():
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🗑️ Barcha yo'lovchi so'zlarini o'chirish", callback_data="delete_all_passenger")],
            [InlineKeyboardButton(text="📋 Yo'lovchi so'zlari", callback_data="list_passenger_words")],
            ...
            [InlineKeyboardButton(text="📋 Haydovchi so'zlari", callback_data="list_driver_words")],
...
async def delete_passenger_words(callback: types.CallbackQuery):
    ...
    if passenger_words:
        ...
        await callback.message.edit_text(
            f"🙋 Yo'lovchi so'zlarini o'chirish:\n\n{words_text}\n\nO'chirish uchun so'zni yozing:"
        )
    else:
        await callback.message.edit_text("📭 Yo'lovchi so'zlari yo'q")

@dp.callback_query(lambda c: c.data == "delete_all_passenger")
async def delete_all_passenger_words(callback: types.CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Sizga ruxsat yo'q!")
        return
    passenger_words = get_keywords('passenger')
    if not passenger_words:
        await callback.answer("📭 Yo'lovchi so'zlari yo'q")
        return
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('DELETE FROM keywords WHERE type = ?', ('passenger',))
            conn.commit()
        await callback.message.edit_text(
            f"✅ Barcha yo'lovchi so'zlari o'chirildi!\n\nJami o'chirilgan: {len(passenger_words)} ta so'z",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_main")]
            ])
        )
        logger.info(f"Admin {callback.from_user.id} barcha yo'lovchi so'zlarini o'chirdi ({len(passenger_words)} ta)")
    except Exception as e:
        logger.error(f"Yo'lovchi so'zlarini o'chirishda xatolik: {e}")
        await callback.answer("❌ Xatolik yuz berdi")

@dp.callback_query(lambda c: c.data == "list_passenger_words")
async def list_passenger_words(callback: types.CallbackQuery):
    passenger_words = get_keywords('passenger')
...
async def delete_all_driver_words(callback: types.CallbackQuery):
    ...
    try:
        with get_db_connection() as conn:
            ...
            conn.commit()
        await callback.message.edit_text(
            f"✅ Barcha haydovchi so'zlari o'chirildi!\n\nJami o'chirilgan: {len(driver_words)} ta so'z",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Orqaga", callback_data="back_main")]
            ])
        )
        ...
        logger.error(f"Haydovchi so'zlarini o'chirishda xatolik: {e}")
        await callback.answer("❌ Xatolik yuz berdi")

@dp.callback_query(lambda c: c.data == "list_driver_words")
async def list_driver_words(callback: types.CallbackQuery):
    driver_words = get_keywords('driver')
...
async def handle_text_message(message: types.Message):
    ...
    if user_id in user_states:
        if user_states[user_id] == 'waiting_passenger_words':
            ...
            del user_states[user_id]
            return
        elif user_states[user_id] == 'waiting_driver_words':
            words = [w.strip() for w in message.text.split(',') if w.strip()]
            for word in words:
                save_keyword('driver', word)
            await message.answer(
                f"✅ {len(words)} ta haydovchi so'zi qo'shildi: {', '.join(words)}\n\nYana so'z yuboring yoki tugatish uchun 'Bekor qilish' bosing.",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="❌ Bekor qilish", callback_data="cancel_add_words")]
                ])
            )
...
PID_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vijdon_bot.pid")
...
def check_single_instance():
    if os.path.exists(PID_FILE):
        try:
            ...
            os.kill(old_pid, 0)  # Process mavjudmi?
            print(f"❌ Bot allaqachon ishlamoqda (PID: {old_pid}). Avval uni to'xtating.")
            raise SystemExit(1)
        except (ProcessLookupError, ValueError, PermissionError):
            pass  # Eski PID fayl yoki boshqa foydalanuvchi jarayoni
